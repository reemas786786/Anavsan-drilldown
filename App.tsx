...
const App: React.FC = () => {
  ...
  const [currentUser, setCurrentUser] = useState<User | null>(usersData[0]); 
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    if (selectedAccount) {
        ...
        items.push({ 
            ...
            onClick: () => setAccountViewPage('Account overview')
        });
    ...
    return items;
  }, [activePage, activeSubPage, selectedAccount, accountViewPage, selectedApplicationId, selectedRecommendation]);
  ...
  const handleAssignQueryTask = (rec: Recommendation) => {
    ...
    // If not a query, or query not found, create a context-aware task object
    if (!assignmentData) {
        assignmentData = {
            id: rec.affectedResource,
            queryText: `Context: ${rec.insightType}\n${rec.message}`, // Use rec details for the "query text" box
            warehouse: 'SYSTEM',
            user: 'anavsan_ai',
            duration: '0s',
  ...
  const renderPage = () => {
    if (selectedAccount) {
      return <AccountView 
        ...
        onUpdateAssignmentStatus={(id, status) => setSidePanel({ type: 'updateAssignmentStatus', data: { id, status }})}
        ...
        <SidePanel isOpen={!!sidePanel} onClose={() => setSidePanel(null)} isFullScreen={sidePanel.type === 'addAccount'} title={sidePanel.type === 'assignQuery' ? 'Assign Optimization Task' : sidePanel.type === 'queryPreview' ? 'Query Preview' : 'Panel'}>
          ...
          {sidePanel.type === 'queryPreview' && (
            <QueryPreviewContent 
                ...
                onOptimize={(q) => { setAnalyzingQuery(q); setAccountViewPage('Query analyzer'); setSidePanel(null); }}
                onSimulate={(q) => { setAnalyzingQuery(q); setAccountViewPage('Query analyzer'); setSidePanel(null); }}
            />
          )}
                ...
                <div className="space-y-4">
                    {(['Assigned', 'In progress', 'Optimized', 'Cannot be optimized', 'Needs clarification'] as AssignmentStatus[]).map(s => (
                        <button 
                            key={s}
                            onClick={() => {
                                setAssignedQueries(prev => prev.map(aq => aq.id === sidePanel.data.id ? { ...aq, status: s } : aq));
                                setSidePanel(null);
                            }}
