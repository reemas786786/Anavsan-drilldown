...
export const queryListData: QueryListItem[] = [
    ...
    { id: 'q-9482105', status: 'Success', costUSD: 8.40, costTokens: 3.2, costCredits: 3.2, duration: '00:01:45', warehouse: 'ANALYTICS_WH', estSavingsUSD: 2.10, estSavingsPercent: 25, queryText: 'WITH cte AS (SELECT * FROM ORDERS) SELECT customer_id, SUM(total) FROM cte GROUP BY 1', timestamp: '2023-11-20T11:15:00Z', type: ['SELECT'], user: 'mike_fin', bytesScanned: 210000000, bytesWritten: 0, severity: 'Medium' },
    { id: 'q-9482106', status: 'Success', costUSD: 15.60, costTokens: 6.8, costCredits: 6.8, duration: '00:04:12', warehouse: 'COMPUTE_WH', estSavingsUSD: 5.20, estSavingsPercent: 33, queryText: 'SELECT p.name, SUM(s.quantity) FROM PRODUCTS p JOIN SALES s ON p.id = s.product_id GROUP BY 1', timestamp: '2023-11-20T12:00:00Z', type: ['SELECT', 'JOIN'], user: 'jane_doe', bytesScanned: 820000000, bytesWritten: 0, severity: 'High' },
    { id: 'q-9482107', status: 'Failed', costUSD: 0.05, costTokens: 0.02, costCredits: 0.02, duration: '00:00:02', warehouse: 'ETL_WH', estSavingsUSD: 0, estSavingsPercent: 0, queryText: 'UPDATE INVENTORY SET status = "out_of_stock" WHERE quantity = 0', timestamp: '2023-11-20T12:30:00Z', type: ['UPDATE'], user: 'system_bot', bytesScanned: 5000, bytesWritten: 0, severity: 'Low' },
    { id: 'q-9482108', status: 'Success', costUSD: 1.20, costTokens: 0.45, costCredits: 0.45, duration: '00:00:25', warehouse: 'LOAD_WH', estSavingsUSD: 0.10, estSavingsPercent: 8, queryText: 'COPY INTO TARGET_TABLE FROM @stage_location FILE_FORMAT = (TYPE = CSV)', timestamp: '2023-11-20T13:10:00Z', type: ['INSERT'], user: 'data_eng_1', bytesScanned: 0, bytesWritten: 45000000, severity: 'Medium' },
    { id: 'q-9482109', status: 'Success', costUSD: 24.50, costTokens: 10.2, costCredits: 10.2, duration: '00:08:50', warehouse: 'COMPUTE_WH', estSavingsUSD: 12.00, estSavingsPercent: 48, queryText: 'SELECT * FROM LOG_DATA WHERE event_type = "ERROR" AND event_time > "2023-10-01"', timestamp: '2023-11-20T14:05:00Z', type: ['SELECT'], user: 'analyst_pro', bytesScanned: 1500000000, bytesWritten: 0, severity: 'High' },
    { id: 'q-9482110', status: 'Success', costUSD: 4.10, costTokens: 1.8, costCredits: 1.8, duration: '00:00:55', warehouse: 'ANALYTICS_WH', estSavingsUSD: 0.50, estSavingsPercent: 12, queryText: 'SELECT region, COUNT(DISTINCT user_id) FROM WEB_TRAFFIC GROUP BY 1', timestamp: '2023-11-20T15:20:00Z', type: ['SELECT', 'Aggregation'], user: 'mike_fin', bytesScanned: 350000000, bytesWritten: 0, severity: 'Medium' },
    { id: 'q-9482111', status: 'Success', costUSD: 0.80, costTokens: 0.3, costCredits: 0.3, duration: '00:00:15', warehouse: 'ETL_WH', estSavingsUSD: 0, estSavingsPercent: 0, queryText: 'DELETE FROM TEMP_STAGING WHERE created_at < "2023-11-01"', timestamp: '2023-11-20T16:00:00Z', type: ['DELETE'], user: 'system_bot', bytesScanned: 120000, bytesWritten: 0, severity: 'Low' },
    { id: 'q-9482112', status: 'Success', costUSD: 42.00, costTokens: 18.5, costCredits: 18.5, duration: '00:15:30', warehouse: 'COMPUTE_WH', estSavingsUSD: 18.00, estSavingsPercent: 42, queryText: 'SELECT c.name, r.revenue FROM CUSTOMERS c JOIN REVENUE_REPORTS r ON c.id = r.customer_id', timestamp: '2023-11-20T17:45:00Z', type: ['SELECT', 'JOIN'], user: 'admin_user', bytesScanned: 3200000000, bytesWritten: 0, severity: 'High' },
    { id: 'q-9482113', status: 'Failed', costUSD: 0.10, costTokens: 0.05, costCredits: 0.05, duration: '00:00:04', warehouse: 'LOAD_WH', estSavingsUSD: 0, estSavingsPercent: 0, queryText: 'SELECT * FROM NON_EXISTENT_TABLE', timestamp: '2023-11-20T18:10:00Z', type: ['SELECT'], user: 'tester_1', bytesScanned: 0, bytesWritten: 0, severity: 'Low' },
    { id: 'q-9482114', status: 'Success', costUSD: 6.30, costTokens: 2.8, costCredits: 2.8, duration: '00:02:10', warehouse: 'ANALYTICS_WH', estSavingsUSD: 1.50, estSavingsPercent: 23, queryText: 'SELECT category, AVG(price) FROM PRODUCTS GROUP BY 1 HAVING AVG(price) > 100', timestamp: '2023-11-20T19:00:00Z', type: ['SELECT', 'Aggregation'], user: 'analyst_pro', bytesScanned: 180000000, bytesWritten: 0, severity: 'Medium' },
    { id: 'q-9482115', status: 'Success', costUSD: 11.20, costTokens: 4.8, costCredits: 4.8, duration: '00:03:45', warehouse: 'COMPUTE_WH', estSavingsUSD: 2.50, estSavingsPercent: 22, queryText: 'SELECT * FROM EVENTS WHERE event_name = "click" AND page_url LIKE "%/checkout%"', timestamp: '2023-11-20T20:15:00Z', type: ['SELECT'], user: 'jane_doe', bytesScanned: 520000000, bytesWritten: 0, severity: 'Medium' },
    { id: 'q-9482116', status: 'Success', costUSD: 0.35, costTokens: 0.15, costCredits: 0.15, duration: '00:00:08', warehouse: 'ETL_WH', estSavingsUSD: 0, estSavingsPercent: 0, queryText: 'INSERT INTO AUDIT_LOG (user, action) VALUES ("admin", "export")', timestamp: '2023-11-20T21:00:00Z', type: ['INSERT'], user: 'admin_user', bytesScanned: 0, bytesWritten: 800, severity: 'Low' },
    { id: 'q-9482117', status: 'Success', costUSD: 19.80, costTokens: 8.4, costCredits: 8.4, duration: '00:07:20', warehouse: 'COMPUTE_WH', estSavingsUSD: 4.00, estSavingsPercent: 20, queryText: 'SELECT DISTINCT email FROM USERS WHERE last_login < "2023-01-01"', timestamp: '2023-11-20T22:30:00Z', type: ['SELECT'], user: 'mike_fin', bytesScanned: 1100000000, bytesWritten: 0, severity: 'High' }
];
