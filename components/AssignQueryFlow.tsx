...
const AssignQueryFlow: React.FC<AssignQueryFlowProps> = ({ query, users, onCancel, onAssign }) => {
    // Only Engineers can be assigned to optimize queries
    ...
    const isNewUser = useMemo(() => {
        if (!assigneeInput || !assigneeInput.includes('@')) return false;
    ...
    const handleSubmit = () => {
        ...
        // Find existing user ID if selected from list
    ...
    return (
        <div className="flex flex-col h-full bg-white">
            <div className="p-8 space-y-8 flex-grow">
                ...
                <div className="space-y-4">
                    <div className="space-y-1">
                        <label className="block text-[13px] font-black text-[#5A5A72] uppercase tracking-wider">Assign Query</label>
                ...
                <div className="space-y-3">
                    <label htmlFor="assignee-input" className="block text-[13px] font-black text-[#5A5A72] uppercase tracking-wider">
                    ...
                    <input
                        ...
                        className="w-full border border-border-light rounded-full px-6 py-4 text-sm focus:ring-1 focus:ring-primary focus:border-primary bg-white placeholder-[#9A9AB2] shadow-sm transition-all"
                ...
                <fieldset className="space-y-3">
                    <legend className="block text-[13px] font-black text-[#5A5A72] uppercase tracking-wider">Assignment Priority</legend>
                            ...
                            <label key={p} className="flex items-center cursor-pointer group">
                                <div className="relative flex items-center justify-center">
                                    <input
                                        ...
                                        className="h-5 w-5 text-primary border-gray-300 focus:ring-primary appearance-none rounded-full border-2 checked:border-primary transition-all"
                                    />
                                    {priority === p && (
                                        <div className="absolute w-2.5 h-2.5 bg-primary rounded-full shadow-sm" />
                                    )}
                                </div>
                ...
                <div className="space-y-3">
                     <label htmlFor="message" className="block text-[13px] font-black text-[#5A5A72] uppercase tracking-wider">Instructions / Context</label>
                     <textarea
                        id="message"
                        rows={6}
                        value={message}
                        onChange={(e) => setMessage(e.target.value)}
                        className="w-full border border-border-light rounded-[24px] px-6 py-4 text-sm focus:ring-1 focus:ring-primary focus:border-primary bg-white placeholder-[#9A9AB2] shadow-sm transition-all resize-none leading-relaxed"
            ...
            {/* Footer */}
            <div className="p-6 bg-[#F9F7FE] border-t border-border-light flex justify-end items-center gap-6 flex-shrink-0">
                <button 
                    onClick={onCancel} 
                ...
                <button 
                    ...
                    className="bg-primary hover:bg-primary-hover text-white font-black px-10 py-3 rounded-full shadow-xl shadow-primary/20 transition-all active:scale-95 text-sm uppercase tracking-wide"
                >
                    Assign to engineer
                </button>
            </div>
